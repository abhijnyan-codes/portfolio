...
export default function About() {
  ...
  // Stats State
  const [prCount, setPrCount] = useState(10); 
    ...
    fetch("/api/github/repos")
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          const completed = data.filter((repo) => repo.topics?.includes("completed"));
          if (completed.length > 0) setProjectCount(completed.length);
        }
      })
